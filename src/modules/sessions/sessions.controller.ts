...
export class SessionsController {
  ...
  async getSessionWithMessages(@Param('id') sessionId: string) {
    const session = await this.sessionsService.getSessionWithMessages(sessionId);
    if (!session) {
      return ApiResponse.error('SESSION_NOT_FOUND', 'Session not found');
    }
    return ApiResponse.success(session, 'Session with messages retrieved successfully');
  }
  ...
  async updateSession(
    ...
    const session = await this.sessionsService.updateSession(sessionId, updateSessionDto);
  ...
  async updateSessionActivity(@Param('id') sessionId: string) {
    ...
    return ApiResponse.success(session, 'Session activity updated successfully');
  }
  ...
  async updateSessionContext(
    ...
    const session = await this.sessionsService.updateSessionContext(sessionId, context);
